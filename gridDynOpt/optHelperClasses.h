...
#define CHECK_OPTIMIATION_FLAG(flag,fname) (flag & (1 << fname))
...
public:
  ...
  optimOffsets (optimOffsets *);
  optimOffsets (const optimOffsets &nOffsets);
...
{
private:
  std::vector<optimOffsets> offsetContainer;       //!< an array of 6 containers for offsets corresponding to the different solver modes
...
public:
  ...
  */
  index_t getaOffset (const optimMode &oMode);
  /** get the voltage offset
  *@param[in] oMode the optimMode we are interested in
  *@return the voltage offset
  */
  index_t getvOffset (const optimMode &oMode);
  /** get the continuous offset
  *@param[in] oMode the optimMode we are interested in
  *@return the diffferential offset
  */
  index_t getContOffset (const optimMode &oMode);
  /** get the integer offset
  *@param[in] oMode the optimMode we are interested in
  *@return the root offset
  */
  index_t getIntOffset (const optimMode &oMode);

  /** get the real generation offset
  *@param[in] oMode the optimMode we are interested in
  *@return the voltage offset
  */
  index_t getgOffset (const optimMode &oMode);

  /** get the reactive generation offset
  *@param[in] oMode the optimMode we are interested in
  *@return the voltage offset
  */
  index_t getqOffset (const optimMode &oMode);
