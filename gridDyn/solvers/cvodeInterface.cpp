...
int cvodeJacDense (long int Neq,
                   ...
                   N_Vector tmp3);
#ifdef KLU_ENABLE
int cvodeJacSparse (realtype ttime,
                    N_Vector state,
                    N_Vector dstate_dt,
                    SlsMat J,
                    void *user_data,
...
int cvodeFunc (realtype ttime, N_Vector state, N_Vector dstate_dt, void *user_data)
{
    ...
    {
        int ret = sd->m_gds->dynAlgebraicSolve (ttime, NVECTOR_DATA (sd->use_omp, state),
    ...
    int ret = sd->m_gds->derivativeFunction (ttime, NVECTOR_DATA (sd->use_omp, state),
                                             NVECTOR_DATA (sd->use_omp, dstate_dt), sd->mode);
    ...
    if (sd->flags[fileCapture_flag])
    {
        ...
        {
            writeVector (ttime, STATE_INFORMATION, sd->funcCallCount, sd->mode.offsetIndex, sd->svsize,
                         NVECTOR_DATA (sd->use_omp, state), sd->stateFile, (sd->funcCallCount != 1));
            writeVector (ttime, DERIVATIVE_INFORMATION, sd->funcCallCount, sd->mode.offsetIndex, sd->svsize,
