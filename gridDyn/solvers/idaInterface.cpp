...
int idaJacDense (long int Neq,
                 ...
                 N_Vector tmp3);
#ifdef KLU_ENABLE
int idaJacSparse (realtype ttime,
                  realtype sD,
                  N_Vector state,
                  N_Vector dstate_dt,
                  N_Vector resid,
...
{IDA_REP_RES_ERR, "The user-provided residual function repeatedly returned a recoverable error flag, but the solver "
...
int idaInterface::solve (coreTime tStop, coreTime &tReturn, step_mode stepMode)
{
    ...
    double tret;
    int retval =
    IDASolve (solverMem, tStop, &tret, state, dstate_dt, (stepMode == step_mode::normal) ? IDA_NORMAL : IDA_ONE_STEP);
    tReturn = tret;
    switch (retval)
    {
    case IDA_SUCCESS:  // no error
...
int idaFunc (realtype ttime, N_Vector state, N_Vector dstate_dt, N_Vector resid, void *user_data)
{
    ...
    // printf("time=%f\n", ttime);
    int ret =
    sd->m_gds->residualFunction (ttime, NVECTOR_DATA (sd->use_omp, state), NVECTOR_DATA (sd->use_omp, dstate_dt),
    ...
    if (sd->flags[fileCapture_flag])
    {
        if (!sd->stateFile.empty ())
        {
            ...
            writeVector (sd->solveTime, DERIVATIVE_INFORMATION, sd->funcCallCount, sd->mode.offsetIndex, sd->svsize,
...
{
    idaInterface *sd = reinterpret_cast<idaInterface *> (user_data);
    sd->m_gds->rootFindingFunction (ttime, NVECTOR_DATA (sd->use_omp, state), NVECTOR_DATA (sd->use_omp, dstate_dt),
                                    gout, sd->mode);
