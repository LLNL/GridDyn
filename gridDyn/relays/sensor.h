...
protected:
  ...
  count_t instructionCounter = 0; //!< the number of instructions the relay has received
	...
	std::vector <basicBlock *> filterBlocks; //!< the filtered blocks
	std::vector<std::shared_ptr<grabberSet>> outGrabbers; //!< Grabbers for the output;
	std::vector<std::shared_ptr<basicBlock>> blkptrs;  //!< storage locations for the shared_ptr of blocks
public:
  ...
  */
  ...
  /** @brief add a shared pointer to gridObject,  must be a basic Block otherwise unrecognized object is thrown
  ...
  */
  ...
  std::shared_ptr<grabberSet> getGrabberSet(index_t kk);
