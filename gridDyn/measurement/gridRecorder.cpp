...
gridRecorder::gridRecorder (coreTime time0, coreTime period) :  collector (time0,period)
{
...
change_code gridRecorder::trigger (coreTime time)
{
  ...
  if ((autosave > 0)&&(dataset.size() >= autosave))
    {
      saveFile ();
      dataset.clear ();
    }
