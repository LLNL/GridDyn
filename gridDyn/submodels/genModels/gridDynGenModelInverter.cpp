...
IOdata gridDynGenModelInverter::getOutputs(const IOdata &args, const stateData &sD, const solverMode &sMode) const
{
	...
	if (rootSize(sMode) > 0)
	{
		...
		if (rootMask[ro] > 0)
		{
			if (opFlags[at_angle_limits])
			{
				...
				algebraicUpdate(args, emptyStateData, m_state.data(), sMode, 1.0);
