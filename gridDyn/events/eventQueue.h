...
private:
  ...
  std::mutex queuelock;  //!< a mutex to protect the queue in case of multi-threaded event insertions
public:
  /** @brief constructor*/
  eventQueue ();
  ...
  */
  ...
  {
    auto ev = std::make_unique < eventTypeAdapter < X >> (newEventObject);
	auto evID = ev->eventID;
  ...
  */
  ...
  {
    auto ev = std::make_unique < eventTypeAdapter < std::shared_ptr<X> >> (std::move(newEventObject));
	auto evID = ev->eventID;
	std::lock_guard<std::mutex> lock(queuelock);
    events.push_back (std::move(ev));
    std::sort (events.begin(), events.end(),compareEventAdapters);
 ...
 */
  ...
  */
  ...
  /** @brief get a vector of all the objects referenced in the events*/
