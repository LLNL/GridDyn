...
void dcBusControls::updateControls()
{
	...
	for (size_t kk = 0; kk < cfrac.size(); ++kk)
	{
		...
		if (pco->checkFlag(remote_power_control))
		{
			...
			{
				controlledBus->log(controlledBus, GD_WARNING_PRINT, "Generator " + pco->getName() + " on indirect path for power control to bus " + controlledBus->getName());
			}
		}
	}
...
void dcBusControls::unmergeBus(dcBus *mbus)
{
	...
	{
		if ((mbus->checkFlag(dcBus::bus_flags::slave_bus)) && (controlledBus->getID() == mbus->busController.masterBus->getID()))
		{
