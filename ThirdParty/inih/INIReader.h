...
class INIReader
{
  public:
    ...
    const std::pair<std::string, std::string> &getAttribute (const std::string &section, int index) const;

  private:
    int _error;
    std::map<std::string, std::string> _values;
    std::map<std::string, std::vector<std::pair<std::string, std::string>>> sectionAttributes;
...
inline const std::string &INIReader::Get (const std::string &section, const std::string &name) const
{
    static const std::string empty;
    std::string key = MakeKey (section, name);
    auto v = _values.find (key);

    return (v != _values.end ()) ? v->second : empty;
}
...
INIReader::Get (const std::string &section, const std::string &name, const std::string &default_value) const
{
    ...
    return (v != _values.end ()) ? v->second : default_value;
}
...
static const std::map<std::string, bool> boolStrings{
  {"true", true},   {"True", true}, {"TRUE", true}, {"tr", true}, {"1", true},      {"T", true},
  {"Y", true},      {"yes", true},  {"YES", true},  {"y", true},  {"false", false}, {"False", false},
  {"FALSE", false}, {"fl", false},  {"0", false},   {"F", false}, {"N", false},     {"no", false},
...
inline bool INIReader::GetBoolean (const std::string &section, const std::string &name, bool default_value) const
{
    ...
    auto val = boolStrings.find (valstr);
    if (val != boolStrings.end ())
    {
        return val->second;
    }
    return default_value;
...
const std::pair<std::string, std::string> &INIReader::getAttribute (const std::string &section, int index) const
{
    static const std::pair<std::string, std::string> emptypair;
    auto v = sectionAttributes.find (section);
    if (v != sectionAttributes.end ())
    {
        if (index < static_cast<int> (v->second.size ()))
        {
...
inline int INIReader::ValueHandler (void *user, const char *section, const char *name, const char *value)
{
    ...
    reader->sectionAttributes[section].emplace_back (name, value);
    return 1;
}
