...
class matrixDataSparseSMB : public matrixData<Y>
{
  ...
  public:
    ...
    */
    ...
    {
        matrixData<Y>::colLim = lim;
        block_computer.setMaxIndex (matrixData<Y>::rowLim, lim);
    }

    void setRowLimit (index_t lim) override
    {
        matrixData<Y>::rowLim = lim;
        block_computer.setMaxIndex (lim, matrixData<Y>::colLim);
    }
    ...
    {
        matrixElement<Y> tp{key_computer.row (cptr->first), key_computer.col (cptr->first), cptr->second};
        ++cptr;
        if (cptr == iend)
        {
            ++ci;
...
class matrixDataSparseSMB<0, X, Y, M> : public matrixData<Y>
{
  ...
  public:
    ...
    @param[in] startCount  the number of elements to reserve
    */
    matrixDataSparseSMB (index_t startCount = 1000) { dVec.reserve (startCount); };
...
*/
template <class X, class Y, sparse_ordering M>
class matrixDataSparseSMB<1, X, Y, M> : public matrixData<Y>
{
    ...
    */
    ...
    virtual matrixElement<Y> element (index_t N) const override
    {
        ...
        ret.row = static_cast<index_t> (
          key_computer.row ((N >= dVec[0].size ()) ? dVec[1][N - dVec[0].size ()].first : dVec[0][N].first));
        ret.col = static_cast<index_t> (
          key_computer.col ((N >= dVec[0].size ()) ? dVec[1][N - dVec[0].size ()].first : dVec[0][N].first));
