...
static const std::map <std::string, std::function<double(double)> >  FuncList1
{
	...
	std::make_pair("randexp", [](double val) {return gridRandom::randNumber(gridRandom::dist_type_t::exponential,val,val); })
	/* *INDENT-ON* */
}; 
...
static const std::map <std::string, std::function<double(double, double)> >  FuncList2
{
	...
	std::make_pair("rand", [](double val1, double val2) {return gridRandom::randNumber(gridRandom::dist_type_t::uniform,val1,val2); }),
	std::make_pair("randn", [](double val1, double val2) {return gridRandom::randNumber(gridRandom::dist_type_t::normal,val1,val2); }),
	std::make_pair("randexp", [](double val1, double val2) {return gridRandom::randNumber(gridRandom::dist_type_t::exponential,val1,val2); }),
	std::make_pair("randlogn", [](double val1, double val2) {return gridRandom::randNumber(gridRandom::dist_type_t::lognormal,val1,val2); }),
	std::make_pair("randint", [](double val1, double val2) {return gridRandom::randNumber(gridRandom::dist_type_t::uniform_int, val1, val2); }),
	std::make_pair("randexval", [](double val1, double val2) {return gridRandom::randNumber(gridRandom::dist_type_t::extreme_value,val1,val2); }),
	std::make_pair("randgamma", [](double val1, double val2) {return gridRandom::randNumber(gridRandom::dist_type_t::gamma,val1,val2); }),
	/* *INDENT-ON* */
};
...
{
	/* *INDENT-OFF* */
	std::make_pair("clamp", [](double val, double valLow, double valHigh) {return (val < valLow) ? valLow : ((val > valHigh) ? valHigh : val); }),
	std::make_pair("max", [](double val1, double val2, double val3) {return (val1 > val2) ? std::max(val1,val3) : std::max(val2,val3); }),
	std::make_pair("min", [](double val1, double val2, double val3) {return (val1 > val2) ? std::min(val2,val3) : std::min(val1,val3); }),
	std::make_pair("hypot", [](double val1, double val2, double val3) {return sqrt(val1*val1+val2*val2+val3*val3); }),
	std::make_pair("mag", [](double val1, double val2, double val3) {return sqrt(val1*val1 + val2*val2 + val3*val3); }),
	...
	std::make_pair("median", [](double val1, double val2,double val3) {return std::max(std::min(val1,val2), std::min(std::max(val1,val2),val3)); }),
	/* *INDENT-ON* */
};
...
static std::map <std::string, std::function<double(const std::vector<double> &)> >  ArrFuncList1
{
	...
	std::make_pair("max", [](const std::vector<double> &ar1) {return *std::max_element(ar1.cbegin(), ar1.cend()); }),
	std::make_pair("min", [](const std::vector<double> &ar1) {return *std::min_element(ar1.cbegin(), ar1.cend()); }),
	...
	std::make_pair("avg", [](const std::vector<double> &ar1) {return (sum(ar1) / static_cast<double> (ar1.size())); }),
...
static const std::map <std::string, std::function<double(const std::vector<double> &, const std::vector<double> &)> >  ArrFuncList2
{
	/* *INDENT-OFF* */
	std::make_pair("vecprod", [](const std::vector<double> &ar1, const std::vector<double> &ar2) {return mult_sum(ar1, ar2); }),
	/* *INDENT-ON* */
};
...
bool isFunctionName(const std::string &ftest, function_type ftype)
{
	...
	{
		auto ifind3 = FuncList3.find(ftest);
...
{
	std::string temp = convertToLowerCase(funcName);
	return mapFind(ArrFuncList2, temp, std::function<double(const std::vector<double> &, const std::vector<double> &)>(nullptr));
