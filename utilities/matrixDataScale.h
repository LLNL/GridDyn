...
public:
	/** @brief constructor
	*/
	matrixDataScale(matrixData<Y> &input, Y scaleFactor):matrixDataContainer<Y>(input),scalingFactor(scaleFactor)
	{
	};

	void assign(index_t row, index_t col, Y num) override
	{
		matrixDataContainer<Y>::ad->assign(row, col, num*scalingFactor);
	};
