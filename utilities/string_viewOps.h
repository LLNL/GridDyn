...
namespace utilities
{
	...
	namespace string_viewOps
	{
		...
		*/
		void trimString(string_view &input, string_view whitespace = whiteSpaceCharacters);
		...
		*/
		...
		@param[in] trimCharacters  the definition of whitespace characters defaults to  \t\n\r\0\v\f
		*/
		...
		*/
		string_view getTailString(string_view input, char sep);
		/** @brief get a stringView that comes after the last of a specified separator
		@param[in] input  the string to separate
		@param[in] sep the separation characters
		@return  the tail string or the string that comes after the last separation character
		if not found returns the entire string
		*/
		string_view getTailString(string_view input, string_view sep);
		...
		*/
		...
		string_viewVector split(string_view str, string_view delimiters = default_delim_chars, delimiter_compression compression = delimiter_compression::off);
		...
		*/
		string_viewVector splitlineQuotes(string_view line, string_view delimiters = default_delim_chars, string_view quoteChars = default_quote_chars, delimiter_compression compression = delimiter_compression::off);
		...
		@return a vector of strings separated by the delimiters characters accounting for bracketing characters
		*/
		string_viewVector splitlineBracket(string_view line, string_view delimiters = default_delim_chars, string_view bracketChars = default_bracket_chars, delimiter_compression compression = delimiter_compression::off);
