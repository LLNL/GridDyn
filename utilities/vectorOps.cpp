...
std::array<double, 3> solve3x3(std::array <std::array<double, 3>, 3> &input, std::array<double, 3> &vals)
{
  ...
  Y[0] = deti * (i11 * vals[0] + i12 * vals[1] + i13 * vals[2]);
  Y[1] = deti * (i21 * vals[0] + i22 * vals[1] + i23 * vals[2]);
  Y[2] = deti * (i31 * vals[0] + i32 * vals[1] + i33 * vals[2]);
...
std::vector<double> interpolateLinear(const std::vector<double> &timeIn, const std::vector<double> &valIn, const std::vector<double> &timeOut)
{
	...
	while (jj<timeOut.size())
	{
		...
		out[jj]= valIn[kk] + (valIn[kk+1] - valIn[kk]) / (timeIn[kk+1] - timeIn[kk])*(timeOut[jj]-timeIn[kk]);
		//out[jj] = std::fma((valIn[kk + 1] - valIn[kk]) / (timeIn[kk + 1] - timeIn[kk]), (timeOut[jj] - timeIn[kk]), valIn[kk]);
		++jj;
	}
breakLoop:
	while (jj<timeOut.size())
	{
		out[jj] = valIn[mx-1] + (valIn[mx-1] - valIn[mx-2]) / (timeIn[mx-1] - timeIn[mx-2])*(timeOut[jj] - timeIn[mx-1]);
		++jj;
	}
	return out;
}