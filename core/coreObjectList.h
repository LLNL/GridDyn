...
struct id {};
struct uid {};
//define a multi-index container based on the object id, which should be unique, and the name which also should be unique, and the user id,
// which isn't necessarily unique.  
using objectIndex = multi_index_container<
	coreObject *,
	indexed_by<
	ordered_unique<tag<id>, const_mem_fun<coreObject, std::uint64_t, &coreObject::getID> >,
