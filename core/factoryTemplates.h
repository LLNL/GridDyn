...
public:
	...
	{}
	virtual std::unique_ptr<parentClass> makeObject()
	{
		return std::make_unique<parentClass>();
	}
...
public:
	...
	explicit childClassFactory(const std::vector<std::string> &names, const std::string &defType) :classFactory<parentClass>(names,defType)
	{
	...
	std::unique_ptr<childClass> makeClassObject()
	{
		return std::make_unique<childClass>();
	}
};
