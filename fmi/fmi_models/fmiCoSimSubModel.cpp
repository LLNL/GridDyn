...
void fmiCoSimSubModel::dynObjectInitializeA(coreTime time, unsigned long flags)
{
...
void fmiCoSimSubModel::dynObjectInitializeB(const IOdata &inputs, const IOdata &desiredOutput, IOdata &inputSet)
{
...
void fmiCoSimSubModel::timestep(coreTime ttime, const IOdata &inputs, const solverMode &sMode)
{
...
/*	double h = localIntegrationTime;
	...
	{
		inputSlope[kk] = (inputs[kk] - prevInput[kk]) / (ttime - prevTime);
	}
	while (time < Tend)
	{
...
void fmiCoSimSubModel::ioPartialDerivatives(const IOdata &inputs, const stateData &sD, matrixData<double> &ad, const IOlocs &inputLocs, const solverMode &sMode)
{
