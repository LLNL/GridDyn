...
public:
	...
	bool isParameter(const std::string &param, fmi_type_t type = fmi_type_t::numeric);
	bool isVariable(const std::string &var, fmi_type_t type = fmi_type_t::numeric);
...
class fmi2ME : public fmi2Object
{
public:
	fmi2ME(fmi2Component cmp, std::shared_ptr<const fmiInfo> info, std::shared_ptr<const fmiCommonFunctions> comFunc, std::shared_ptr<const fmiMEFunctions> MEFunc);
	...
	std::shared_ptr<const fmiMEFunctions> getModelExchangeFunctions() const
	{
		return MEFunctions;
	}
...
private:
	...
	std::shared_ptr<const fmiMEFunctions> MEFunctions;
};

/** class containing the Information for working with a FMI coSimulation object*/
class fmi2CoSim : public fmi2Object
{
public:
	fmi2CoSim(fmi2Component cmp, std::shared_ptr<const fmiInfo> info, std::shared_ptr<const fmiCommonFunctions> comFunc, std::shared_ptr<const fmiCoSimFunctions> csFunc);
	void SetRealInputDerivatives(const fmi2ValueReference vr[], size_t nvr, const fmi2Integer[], const fmi2Real[]);
	void getRealOutputDerivatives(const fmi2ValueReference vr[], size_t nvr, const fmi2Integer[], fmi2Real[]) const;
