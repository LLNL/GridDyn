...
runner_t getFmiRunner (fmi2Component comp)
{
    ...
    auto &runner = (p->first < fmiRunnerInstances.size ()) ? fmiRunnerInstances[p->first] : fmiRunnerInstances[0];
...
fmi2Status
fmi2SetDebugLogging (fmi2Component comp, fmi2Boolean loggingOn, size_t nCategories, const fmi2String categories[])
{
                               ...
                               fmi2Boolean loggingOn)
{
    if ((fmuType == fmi2Type::fmi2CoSimulation) || (fmuType == fmi2Type::fmi2ModelExchange))
    {
        ...
        try
        {
            auto fmiM = std::make_unique<fmi::fmiRunner> (instanceName, locString, functions,
