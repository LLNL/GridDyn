...
public:
	...
	void updateObject(const coreObject *newobj)
	{
...
public:
	...
	unrecognizedParameter(const std::string &param) :std::invalid_argument(std::string("unrecognized Parameter:")+param) {};
};
...
public:
	...
	invalidParameterValue(const std::string &param) :std::invalid_argument(std::string("invalid parameter value for ")+param) {};
};
...
private:
	std::string message;
public:
	explicit executionFailure(const coreObject *obj, const std::string &error_message) :coreObjectException(obj), message(error_message)
	{};
	virtual const char *what() const noexcept override
	{
