...
{
  protected:
    FMItype *fmisub = nullptr;  //!< a pointer to an fmi submodel of some kind either FMIMESubModel or FMICoSimSubmodel
  ...
  public:
	...
	@details this defaults to the regular input strings call but there are a few reasons that this could be distinct
	from the general object input and output strings so a separate call is provided with the default to the regular call
	...
	*/
	...
	@details this defaults to the regular output strings call but there are a few reasons that this could be distinct
	from the general object input and output strings so a separate call is provided with the default to the regular call
	@return a reference to a vector of string vectors containing the potential names of the inputs
	*/
    ...
    void setupFmiIo ()
    {
       ...
       //Make sure the specified sizes are big enough
        ...
        for (index_t ii = 0; ii < BaseObj::m_inputSize; ++ii)
        {
            ...
            {
                int ind = findCloseStringMatch (iNames[ii], istrings, string_match_close);
                if (ind >= 0)
                {
                ...
                else  // now try some alternative representations
                {
                    ...
                    altInames.emplace_back ("in[" + std::to_string (ii) + "]");
                    altInames.emplace_back ("input[" + std::to_string (ii) + "]");
                    ind = findCloseStringMatch (altInames, istrings, string_match_close);
                    if (ind >= 0)
                    {
                        inputNames_actual[ii] = istrings[ind];
                    }
            ...
            {
                int ind = findCloseStringMatch ({inputNames_specified[ii]}, istrings, string_match_exact);
                if (ind >= 0)
                {
                    inputNames_actual[ii] = istrings[ind];
                }
                ...
                {
                    ind = findCloseStringMatch ({inputNames_specified[ii], inputNames_specified[ii] + "_in"},
                                                istrings, string_match_close);
                    if (ind >= 0)
                    {
                        inputNames_actual[ii] = istrings[ind];
                    }
        ...
        for (index_t ii = 0; ii < BaseObj::m_outputSize; ++ii)
        {
            ...
            {
                int ind = findCloseStringMatch (oNames[ii], ostrings, string_match_close);
                if (ind >= 0)
                {
                ...
                else  // now try some alternative representations
                {
                    ...
                    ind = findCloseStringMatch (altOnames, ostrings, string_match_close);
                    if (ind >= 0)
                    {
                        outputNames_actual[ii] = ostrings[ind];
                    }
            ...
            else
            {
                ...
                {
                    ind = findCloseStringMatch ({outputNames_specified[ii], outputNames_specified[ii] + "_out"},
                                                ostrings, string_match_close);
                    if (ind >= 0)
                    {
                        outputNames_actual[ii] = ostrings[ind];
                    }
                    ...
                    {
						BaseObj::log(this, print_level::warning, "unable to match inputs for output" + std::to_string (ii));
                    }
                }
            }
        }
    ...
    void set (const std::string &param, const std::string &val) override
    {
        ...
        else if (param2.compare (param2.size () - 3, 2, "in") == 0)
        {
            ...
            {
                int ind =
                  findCloseStringMatch ({param2.substr (0, param2.size () - 2), param2}, istr, string_match_close);
                if (ind >= 0)
                {
        ...
        else if (param2.compare (param2.size () - 4, 2, "out") == 0)
        {
            ...
            {
                int ind =
                  findCloseStringMatch ({param2.substr (0, param2.size () - 3), param2}, ostr, string_match_close);
                if (ind >= 0)
                {
        ...
        else
        {
            if (!BaseObj::opFlags[pFlow_initialized])
            {
                for (auto &istr : this->inputNames ())
                {
                    int ind = findCloseStringMatch ({param2}, istr, string_match_close);
                    if (ind >= 0)
                    {
                        inputNames_specified[ind] = val;
                        return;
                ...
                for (auto &ostr : this->outputNames ())
                {
                    int ind = findCloseStringMatch ({param2}, ostr, string_match_close);
                    if (ind >= 0)
                    {
                        outputNames_specified[ind] = val;
                        return;
