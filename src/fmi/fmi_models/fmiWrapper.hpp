    ...
    */
        ...
        for (index_t ii = 0; ii < BaseObj::m_inputSize; ++ii)
        {
            if (inputNames_specified[ii].empty())
            {
                ...
                else  // now try some alternative representations
                {
                    ...
                    {
                        BaseObj::log(this, print_level::warning,
            ...
            {
                int ind = findCloseStringMatch({inputNames_specified[ii]}, istrings, string_match_type::exact);
                if (ind >= 0)
                {
                    inputNames_actual[ii] = istrings[ind];
                }
                else
                {
                    ind = findCloseStringMatch({inputNames_specified[ii], inputNames_specified[ii] + "_in"},
                                               istrings, string_match_type::close);
                    if (ind >= 0)
                    {
                        inputNames_actual[ii] = istrings[ind];
                    }
            ...
            else
            {
                ...
                else
                {
                    ind = findCloseStringMatch({outputNames_specified[ii], outputNames_specified[ii] + "_out"},
                                               ostrings, string_match_type::close);
                    if (ind >= 0)
                    {
                        outputNames_actual[ii] = ostrings[ind];
                    }
    ...
    void set(const std::string &param, const std::string &val) override
    {
        ...
        else if (param2.compare(param2.size() - 3, 2, "in") == 0)
        {
            ...
            {
                int ind = findCloseStringMatch({param2.substr(0, param2.size() - 2), param2}, istr,
        ...
        else if (param2.compare(param2.size() - 4, 2, "out") == 0)
        {
            ...
            {
                int ind = findCloseStringMatch({param2.substr(0, param2.size() - 3), param2}, ostr,
                                               string_match_type::close);
                if (ind >= 0)
                {
