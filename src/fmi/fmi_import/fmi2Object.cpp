...
fmi2Object::~fmi2Object () { commonFunctions->fmi2FreeInstance (comp); }
void fmi2Object::setupExperiment (fmi2Boolean toleranceDefined,
                                  ...
                                  fmi2Real stopTime)
{
    auto ret = commonFunctions->fmi2SetupExperiment (comp, toleranceDefined, tolerance, startTime, stopTimeDefined,
                                                     stopTime);
...
void fmi2Object::getDirectionalDerivative (const fmi2ValueReference vUnknown_ref[],
                                           ...
                                           fmi2Real dvUnknown[])
{
    auto ret = commonFunctions->fmi2GetDirectionalDerivative (comp, vUnknown_ref, nUnknown, vKnown_ref, nKnown,
                                                              dvKnown, dvUnknown);
    if (ret != fmi2Status::fmi2OK)
    {
        handleNonOKReturnValues (ret);
...
{
    double dy;
    commonFunctions->fmi2GetDirectionalDerivative (comp, &(info->getVariableInfo (index_x).valueRef), 1,
                                                   &(info->getVariableInfo (index_y).valueRef), 1, &dx, &dy);
    return dy;
}
...
void fmi2Object::setDefaultInputs () { setInputVariables (info->getVariableNames ("input")); }

void fmi2Object::setDefaultOutputs () { setOutputVariables (info->getVariableNames ("output")); }

void fmi2Object::setInputs (const fmi2Real inputs[])
{
    auto ret =
      commonFunctions->fmi2SetReal (comp, activeInputs.getValueRef (), activeInputs.getVRcount (), inputs);
    if (ret != fmi2Status::fmi2OK)
    {
        handleNonOKReturnValues (ret);
    }
...
{
    auto ret =
      commonFunctions->fmi2GetReal (comp, activeInputs.getValueRef (), activeInputs.getVRcount (), inputs);
    if (ret != fmi2Status::fmi2OK)
    {
        handleNonOKReturnValues (ret);
    }
...
{
    auto ret =
      commonFunctions->fmi2GetReal (comp, activeOutputs.getValueRef (), activeOutputs.getVRcount (), outputs);
    if (ret != fmi2Status::fmi2OK)
    {
        handleNonOKReturnValues (ret);
    }
...
fmiVariableSet fmi2Object::getVariableSet (int index) const { return info->getVariableSet (index); }
