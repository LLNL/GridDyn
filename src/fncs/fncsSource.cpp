...
{
    if (!valKey.empty()) {
        std::string def = std::to_string(gridUnits::unitConversion(
            m_output / scaleFactor, outputUnits, inputUnits, systemBasePower));
        fncsRegister::instance()->registerSubscription(valKey,
