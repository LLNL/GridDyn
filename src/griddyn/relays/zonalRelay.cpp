...
*/
    ...
    void zonalRelay::dynObjectInitializeA(coreTime time0, std::uint32_t flags)
    {
        ...
        for (index_t kk = 0; kk < m_zones; ++kk) {
            if (opFlags[nondirectional_flag]) {
                ...
                add(std::shared_ptr<Condition>(
                    make_condition("admittance" + std::to_string(m_terminal),
                                   ">=",
                                   1.0 / (m_zoneLevels[kk] * baseImpedance),
                                   m_sourceObject)));
            }
    ...
    void zonalRelay::receiveMessage(std::uint64_t /*sourceID*/,
    ...
    std::string zonalRelay::generateAutoName(int code)
    {
        ...
        switch (code) {
            ...
            case 2:
                if (m_terminal == 1) {
                    autoname =
                        std::to_string(b1->getUserID()) + '_' + std::to_string(b2->getUserID());
                } else {
                    autoname =
                        std::to_string(b2->getUserID()) + '_' + std::to_string(b1->getUserID());
                }
                break;
            default:;
                // do nothing
