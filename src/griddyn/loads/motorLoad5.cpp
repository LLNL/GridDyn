...
namespace loads {
    ...
    void motorLoad5::converge()
    {
        ...
        while (err > 1e-6) {
            ...
            gmlc::utilities::solve2x2(fbs,
                                      -1.0 / T0pp,
                                      1.0 / T0pp,
                                      fbs,
                                      fbs * erp - erp / T0pp + (xp - xpp) / T0pp * ir,
                                      fbs * emp - emp / T0pp + (xp - xpp) / T0pp * im,
                                      er,
                                      em);
                              ...
                              const solverMode& sMode)
    {
        if (isDynamic(sMode)) {
            ...
            // Erp and Emp
            rv[erpA] =
                systemBaseFrequency * slip * gm[empA] - (gm[erpA] + (x0 - xp) * gm[imA]) / T0p;
            rv[empA] =
                -systemBaseFrequency * slip * gm[erpA] - (gm[empA] - (x0 - xp) * gm[irA]) / T0p;
    ...
    void motorLoad5::updateCurrents(const IOdata& inputs,
                                    const stateData& sD,
                                    const solverMode& sMode)
    {
        ...
        gmlc::utilities::solve2x2(r,
                                  ...
                                  xpp,
                                  r,
                                  vr - Loc.diffStateLoc[3],
                                  vm - Loc.diffStateLoc[4],
                                  Loc.destLoc[0],
                                  Loc.destLoc[1]);
    }
                                ...
                                const solverMode& sMode)
    {
        ...
        dv[empD] =
            -systemBaseFrequency * slip * dst[erpD] - (dst[empD] - (x0 - xp) * ast[irA]) / T0p;
