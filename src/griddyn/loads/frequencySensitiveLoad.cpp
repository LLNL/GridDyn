...
namespace loads {
    ...
    void frequencySensitiveLoad::timestep(coreTime time,
                                          const IOdata& inputs,
    ...
    {
        getParamString<frequencySensitiveLoad, Load>(
    ...
    {
        if (inputs
                .empty())  // we only have output derivatives if the input arguments are not counted
        {
    ...
    count_t frequencySensitiveLoad::outputDependencyCount(index_t num,
                                                          const solverMode& sMode) const
    {
