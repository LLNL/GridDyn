...
public:
  ...
  void setMessageType (std::uint32_t type)
  {
    m_messageType = type;
  }
...
public:
  ...
  /** build a message payload of the specific type, deriving the general type from the valid ranges of specific types defining in the factory
  @param type the specific message code for the message*/
...
{
  static_assert (std::is_base_of<CommPayload, PayloadType>::value, "factory class must have commMessage as base");
public:
  ...
  std::shared_ptr<PayloadType> makeTypePayload ()
  {
    return(std::make_shared<PayloadType> ());
  }
