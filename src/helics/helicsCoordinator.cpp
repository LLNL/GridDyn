...
std::shared_ptr<helics::Federate> helicsCoordinator::RegisterAsFederate()
{
	...
	for (auto &p : pubs_)
	{
		...
		{
			p.id = vFed_->registerGlobalPublication(p.name, to_string(p.type), gridUnits::to_string(p.unitType));
		}
		else
		{
	...
	for (auto &s : subs_)
	{
		...
		{
			s.id = vFed_->registerOptionalSubscription(s.name, to_string(s.type), gridUnits::to_string(s.unitType));
		}
		else
		{
...
int32_t helicsCoordinator::addPublication(const std::string &pubName, helicsValueType type, gridUnits::units_t unitType)
{
	PubInfo p;
	p.name = pubName;
	p.type = type;
...
int32_t helicsCoordinator::addSubscription(const std::string &subName, helicsValueType type, gridUnits::units_t unitType)
{
	SubInfo s;
	s.name = subName;
	s.type = type;
...
void helicsCoordinator::updatePublication(int32_t index, const std::string &pubName, helicsValueType type, gridUnits::units_t unitType)
{
...
void helicsCoordinator::updateSubscription(int32_t index, const std::string &subName, helicsValueType type, gridUnits::units_t unitType)
{
	if ((index >= 0) && (index < static_cast<int32_t>(subs_.size())))
	{
		if (!subName.empty())
