...
#include "paradae/drivers/braid_driver.h"
...
std::shared_ptr<solverInterface> braidSolver::clone(std::shared_ptr<solverInterface> si, bool fullCopy) const
{
	auto rp = cloneBase<braidSolver, solverInterface>(this, si, fullCopy);
	if (!rp)
	{
		return si;
	}
