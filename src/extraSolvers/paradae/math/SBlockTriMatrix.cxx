...
{
  m=m_;isfacto=false;isrankdef=false;
  s=s_;isdense=isdense_;
  nb_blocks=(m*(m-1))/2;
  diag=new Matrix*[1];
...
Real& SBlockTriMatrix::operator()(int i, int j)
{
  ...
  if ( i<0 || i>=s*m || j<0 || j>=s*m || ib<jb )
    {
      cerr << "Bad index in SBlockTriMatrix[" << m << "]::operator(" << i << "," << j << ")" << endl;
      abort();
    }
  if (isfacto)
    {
...
void SBlockTriMatrix::SetSubMat(int i, int j, const VirtualMatrix& mat, Real multcoeff)
{
  ...
  if (i<0 || j<0 || i>=m || j>=m || i<j)
    {
      cerr << "Bad index in SBlockTriMatrix[" << m << "," << s << "]::SetBlock(" << i << "," << j << ")" << endl;
      abort();
    }
  if (s!=mat.GetM())
    {
      cerr << "Inserting VirtualMatrix[" << mat.GetM() << "] into SBlockTriMatrix[" << m << "," << s << "]::SetBlock(" << i << "," << j << ")" << endl;
      abort();
    }
...
void SBlockTriMatrix::AXPBY(Real alpha, Real beta, const VirtualMatrix& x)
{
  if(m!=x.GetM())
    {
      cerr << "Bad index in SBlockTriMatrix[" << m << "]::AXPBY=(VirtualMatrix[" << x.GetM() << "])" << endl;
      abort();
    }
  if (isfacto)
    {
...
void SBlockTriMatrix::MatMult(Vector& b, bool transpose) const
{
  if (b.GetM()!=s*m)
    {
      cerr << "Bad index in SBlockTriMatrix[" << m << "," << s << "]::MatMult(Vector[" << b.GetM() << "])" << endl;
      abort();
    }
