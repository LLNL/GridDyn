...
void SMultiVector::GetPVector(int i, PVector& v) const
{
  ...
  if ( i<0 || i>=ns )
    {
      cerr << "Error in SMultiVector[" << ns << "," << nx << "]::GetPVector(" << i << ")" << endl;
      abort();
    }
#endif
  v.Set(nx,data+i*nx);
...
void SMultiVector::GetSVector(int i, SVector& v) const
{
  ...
  if ( i<0 || i>=ns )
    {
      cerr << "Error in SMultiVector[" << ns << "," << nx << "]::GetSVector(" << i << ")" << endl;
      abort();
    }
#endif
  v.Resize(nx);
...
void SMultiVector::PushFront(const SVector& v)
{
  ...
  if ( nx!=v.GetM() )
    {
      cerr << "Error in SMultiVector[" << ns << "," << nx << "]::PushFront(v[" << v.GetM() << "])" << endl;
      abort();
    }
#endif
...
void SMultiVector::PushAndPop(const SVector& v)
{
  ...
  if ( ns==0 || nx!=v.GetM() )
    {
      cerr << "Error in SMultiVector[" << ns << "," << nx << "]::PushAndPop(v[" << v.GetM() << "])" << endl;
      abort();
    }
#endif
  if ( ns>1 )
