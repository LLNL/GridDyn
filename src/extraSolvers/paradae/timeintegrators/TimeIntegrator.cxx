  ...
  if (equation->HasEvents())
    {
      ...
      int nx=size_x;
      SMultiVector t_y(ns+1,1);PMultiVector pt_y(t_y);
      SMultiVector v_y(ns+1,nx);PMultiVector pv_y(v_y);
      SMultiVector t_dy(2,1);PMultiVector pt_dy(t_dy);
      SMultiVector v_dy(2,nx);PMultiVector pv_dy(v_dy);
      ...
      for (int i=0;i<ns;i++)
	{
	  ...
	  v_y.GetPVector(i,xtmp2);xtmp2.CopyData(xtmp1);
	}
      t_y(ns)=thi;
      v_y.GetPVector(ns,xtmp2);xtmp2.CopyData(val.xnext);

      t_dy(0)=tlo;
      t_dy(1)=thi;
      v_dy.GetPVector(0,xtmp2);xtmp2.CopyData(val.dxprev);
      v_dy.GetPVector(1,xtmp2);xtmp2.CopyData(val.dxnext);
...
RCODE TimeIntegrator::PostStep(DATA_Struct& val, bool success_solver, bool found_root, bool success_error_test)
{
   // JBS HACK
   if (val.used_dt < 1e-5)
      success_error_test=1;
 
  if (!success_solver)
    {
      /* Newton solver failed, we reset the *next values to *prev, and set an arbitrary rfactor */
      val.RollBack();
      val.next_dt=val.used_dt/max_rfactor; // TODO : wich rfactor should we use?
      return NONLIN_FAIL;
    }
