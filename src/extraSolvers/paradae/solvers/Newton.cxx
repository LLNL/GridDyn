...
int Newton::Solve(Solver_App* app, Vector& x)
{
  ...
  if (std::isnan(residual_fx) || std::isnan(residual_dx) || std::isinf(residual_fx) || std::isinf(residual_dx))
    {
      cerr << "Newton exit on BAD failure : res_fx = " << residual_fx << ", res_dx = " << residual_dx << endl;
#ifdef DEBUG_NEWTON
      ofstream file;file.open("newton_hist");
    ...
    {
      cerr << "Newton exit on failure : res_fx = " << residual_fx << ", res_dx = " << residual_dx << endl;
#ifdef DEBUG_NEWTON
      ofstream file;file.open("newton_hist");
      file << setprecision(20);
      PVector hi;
