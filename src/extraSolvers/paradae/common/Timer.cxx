...
void Timer::Start(string key, string output, string dep)
{
  ...
  if (it_value == this->end())
      ...
      if (nb_elems<MAX_NTIME)
	{
	  ...
	  sstr=(sstr>3*depth[nb_elems]+output.length())?sstr:(3*depth[nb_elems]+output.length());
...
Real Timer::GetCurrentTime(string key)
{
  auto it_value=this->find(key);
  if (it_value != this->end())
    return elems[it_value->second].GetTime();
  return -1;
}
      ...
      for (int i=0;i<nb_elems;i++)
	{
	  if (depends_on[i]==deps)
	      ...
	      if (deps>-1)
		{
	      ...
	      printf(str.str().c_str(),elems[i].GetName().c_str(),min_t(i),avg_t(i),max_t(i));
	      this->ShowStats(i);
	    }
	}
    }
