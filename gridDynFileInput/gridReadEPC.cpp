...
void ProcessSectionObject (std::string line,
                           ...
                           std::vector<gridBus *> &busList,
                           std::function<void(X *, string_view)> Func)
{
    int cnt = getSectionCount (line);
...
void loadEPC (coreObject *parentObject, const std::string &filename, const basicReaderInfo &bri)
{
    ...
    */
    ...
    while (nextLine (file, line))
    {
        ...
        {
            ProcessSectionObject<gridDynGenerator> (line, file, "generator", busList,
                                                    [base](gridDynGenerator *gen, string_view config) {
...
//#generator data  [4141]    id   ------------long_id_------------    st ---no--     reg_name       prf  qrf  ar
