...
void loadBusArray (coreObject *parentObject, double basepower, mArray &buses, std::vector<gridBus *> &busList, const basicReaderInfo &)
{
  ...
  gridLoad *ld = nullptr;
  auto busFactory = dynamic_cast<typeFactory<gridBus> *> (coreObjectFactory::instance ()->getFactory ("bus")->getFactory (""));
  busFactory->prepObjects (static_cast<count_t> (buses.size ()), parentObject);

  auto loadFactory = dynamic_cast<typeFactory<gridLoad> *> (coreObjectFactory::instance ()->getFactory ("load")->getFactory (""));
  ...
  for (const auto &busData:buses)
    {
      ...
      if (busList[ind1 - 1] == nullptr)
        {
          busList[ind1 - 1] = busFactory->makeTypeObject ();
          busList[ind1 - 1]->set ("basepower", basepower);
          busList[ind1 - 1]->setName ( "Bus_" + std::to_string (ind1));
          busList[ind1 - 1]->setUserID (ind1);
...
int loadGenArray (coreObject *parentObject,  mArray &gens, std::vector<gridBus *> &busList, const basicReaderInfo &)
{
  ...
  auto genFactory = dynamic_cast<typeFactory<gridDynGenerator> *> (coreObjectFactory::instance ()->getFactory ("generator")->getFactory (""));
  ...
  for (auto &genLine : gens)
    {
      ...
      gridBus *bus = busList[ind1 - 1];
      gridDynGenerator *gen = genFactory->makeTypeObject ();
          ...
          if ((genLine[10] != 0)&&(genLine[11] != 0))
            {
              std::vector<double> PC {
                genLine[10],genLine[11]
              };
              std::vector<double> Qmin {
                genLine[12], genLine[14]
              };
              std::vector<double> Qmax {
                genLine[13], genLine[15]
              };
...
void loadGenCostArray (coreObject *parentObject, mArray &genCost, int gencount)
{
  ...
  auto genOptFactory = dynamic_cast<optObjectFactory<gridGenOpt, gridDynGenerator> *> (coreOptObjectFactory::instance ()->getFactory ("")->getFactory ("generator"));
  ...
  for (const auto &linkData:lnks)
    {
      ...
      gridLink *lnk = linkFactory->makeTypeObject ();
      ++kk;
      lnk->setUserID (kk);
