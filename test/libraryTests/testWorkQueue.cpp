...
BOOST_AUTO_TEST_CASE(workQueue_test1)
{
	...
	auto fk = [] {std::this_thread::sleep_for(std::chrono::milliseconds(100)); return std::hash<std::thread::id>()(std::this_thread::get_id()); };
...
BOOST_AUTO_TEST_CASE(workQueue_test2)
{
	...
	BOOST_CHECK(elapsed_time.count() >= 0.1);
