...
BOOST_FIXTURE_TEST_SUITE(input_tests,
                       ...
                       caseIndex)
{
    ...
    std::function<void(size_t, double, double)> vfunc = [=](size_t index, double v1, double v2) {
        std::cout << mp.first << " Voltage difference bus " << index + 1 << "::" << v1 << " vs. "
                  << v2 << "::" << v1 - v2 << '\n';
    };
    ...
    std::function<void(size_t, double, double)> afunc = [=](size_t index, double v1, double v2) {
        std::cout << mp.first << " Angle difference bus " << index + 1 << "::" << v1 * 180.0 / kPI
                  << " vs. " << v2 * 180.0 / kPI << "::" << (v1 - v2) * 180.0 / kPI << '\n';
    };
    ...
    std::function<void(size_t, double, double)> Pfunc = [=](size_t index, double v1, double v2) {
        std::cout << mp.first << " Power difference-- bus " << index + 1 << "::" << v1 << " vs. "
                  << v2 << '\n';
    };
    ...
    */
    ...
    if (mp.first ==
        "ieee300.cdf") {  // need to do a custom reset for the peculiarities of this system
...
BOOST_DATA_TEST_CASE_F(gridDynSimulationTestFixture, compare_cases, data::xrange(5), caseIndex)
{
    ...
    for (size_t ns = 1; ns < caseSet.size(); ++ns) {
        ...
        BOOST_REQUIRE_MESSAGE(gds2->currentProcessState() ==
